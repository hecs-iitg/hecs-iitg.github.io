// HECS IITG — progressive enhancement only.
// The institutional masthead is static markup in every page and its styles live
// in style.css. Nothing in this file is required for the site to render or for
// the content to be readable; if it fails to load, the pages still work.

// Marks the document as scripted so CSS can opt into JS-only behaviour
// (scroll reveals start hidden only when there is something to reveal them).
...
// --- Hero particles, generated locally with no external dependency ----------
...
if (particleHost && !reduceMotion) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    p.style.setProperty('--x', (4 + Math.random() * 92).toFixed(1) + '%');
    p.style.setProperty('--y', (5 + Math.random() * 88).toFixed(1) + '%');
    p.style.setProperty('--opacity', (0.18 + Math.random() * .58).toFixed(2));
    p.style.setProperty('--blur', (Math.random() * 1.2).toFixed(1) + 'px');
    p.style.setProperty('--duration', (4.5 + Math.random() * 7).toFixed(1) + 's');
...
if (logoPanel && finePointer && !reduceMotion) {
  logoPanel.addEventListener('pointermove', e => {
    const r = logoPanel.getBoundingClientRect();
    const x = (e.clientX - r.left) / r.width;
    const y = (e.clientY - r.top) / r.height;
    logoPanel.style.setProperty('--tilt-y', `${((x - .5) * 7).toFixed(2)}deg`);
    logoPanel.style.setProperty('--tilt-x', `${((.5 - y) * 6).toFixed(2)}deg`);
  });
...
if (finePointer && !reduceMotion) {
  interactive.forEach(el => {
    el.addEventListener('pointermove', e => {
      ...
      el.style.transform = `perspective(900px) rotateX(${((.5 - y) * 2.8).toFixed(2)}deg) rotateY(${((x - .5) * 3.2).toFixed(2)}deg) translateY(-3px)`;
    });
...
const pubEmpty   = document.querySelector('#pub-empty');
...
function applyPubFilters() {
  ...
  let shown = 0;
  pubs.forEach(p => {
    const match = (active === 'all' || p.dataset.type === active) &&
...
// --- Contact form ------------------------------------------------------------
// The form also works without this: it carries a real action and access key, so
// a no-JS submit posts straight to Web3Forms. This only upgrades it to an
...
if (form) {
  ...
  form.addEventListener('submit', async e => {
    ...
    data.set('subject', `HECS IITG enquiry: ${topic}`);
    data.set('from_name', 'HECS IITG website');
    ...
    try {
      ...
      form.reset();
      setFormStatus('Thank you — your enquiry has been sent. You will get a reply at the address you gave.', 'success');
    } catch (error) {
      setFormStatus('We could not send your enquiry. Please try again, or email dkumar39@wisc.edu directly.', 'error');
    } finally {
